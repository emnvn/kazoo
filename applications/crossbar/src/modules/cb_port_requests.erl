...
-define(ACCOUNTS_NAME_LIST, <<"accounts/name_list">>).
...
get_account_names(Keys) ->
    case couch_mgr:get_results(?WH_ACCOUNTS_DB, ?ACCOUNTS_NAME_LIST, Keys) of
        {'ok', JObjs} ->
            [{wh_json:get_value(<<"id">>, JObj), wh_json:get_value(<<"value">>, JObj)}
             || JObj <- JObjs
            ];
        {'error', _} -> []
    end.
